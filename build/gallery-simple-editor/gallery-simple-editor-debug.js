YUI.add('gallery-simple-editor', function(Y) {
    ...
    */
    ...
    var getButton = function(id) {
        ...
        if (id instanceof Y.Toolbar.Button) {
            button = id;
        } else {
    ...
    */
    ...
    Toolbar.ATTRS = {
        ...
        */
        buttonType: {
            ...
            setter: function(type) {
                this.buttonType = Y.Toolbar.Button;
            }
        },
        /**
        * @attribute buttons
        ...
        */
        grouplabels: {
            value: true,
            setter: function(grouplabels) {
                if (grouplabels) {
                    this.get('cont').removeClass(this.CLASS_PREFIX + '-nogrouplabels');
                } else {
                    this.get('cont').addClass(this.CLASS_PREFIX + '-nogrouplabels');
                }
            }
        },
    ...
    Y.extend(Toolbar, Y.Base, {
        ...
        _handleCollapse: function() {
            ...
            if (this._titlebar) {
                var collapseEl = null;
                var el = this._titlebar.query('span.collapse');
                if (collapse) {
                    if (el) {
                        //There is already a collapse button
                        return true;
                    }
                    collapseEl = Y.Node.create('<span title="' + this.STR_COLLAPSE + '" class="collapse">X</span>');
                    this._titlebar.appendChild(collapseEl);
                    collapseEl.on('click', Y.bind(function() {
                        if (this.get('cont').get('parentNode').hasClass('yui-toolbar-container-collapsed')) {
                    ...
                    collapseEl = this._titlebar.query('span.collapse');
                    if (collapseEl) {
                        if (this.get('cont').get('parentNode').hasClass('yui-toolbar-container-collapsed')) {
                            //We are closed, reopen the titlebar..
                            this.collapse(false); //Expand Toolbar
                        }
                        collapseEl.get('parentNode').removeChild(collapseEl);
                    }
                }
            }
        },
        _renderTitle: function() {
            ...
            if (titlebar) {
                if (this._titlebar && this._titlebar.get('parentNode')) {
                    this._titlebar.get('parentNode').removeChild(this._titlebar);
                }
                this._titlebar = Y.Node.create('<div tabIndex="-1" class="' + this.CLASS_PREFIX + '-titlebar"></div>');
                this._titlebar.on('focus', Y.bind(this._handleFocus, this));

                if (Y.Lang.isString(titlebar)) {
                    var h2 = Y.Node.create('<h2 tabIndex="-1"><a href="#" tabIndex="0">' + titlebar + '</a></h2>');
                    this._titlebar.appendChild(h2);
                    h2.get('firstChild').on('click', function(ev) {
                        ev.halt();
                    });
                    /*
                ...
                if (this.get('node').get('firstChild')) {
                    this.get('node').insertBefore(this._titlebar, this.get('node').get('firstChild'));
                } else {
                    this.get('node').appendChild(this._titlebar);
                }
...
/* }}} */
        ...
        */
        initializer: function() {
            var fs = Y.Node.create('<fieldset><legend>Toolbar</legend><div class="yui-toolbar-subcont"></div></fieldset>');
            this.set('id', this.get('node').get('id'));
            this.get('node').appendChild(fs);
            this.set('cont', fs.query('div.yui-toolbar-subcont'));
            this.get('node').set('tabIndex', '-1');
            this.get('node').addClass(this.CLASS_CONTAINER);

            this._renderTitle();
            this._handleCollapse();
            this.on('titlebarChange', Y.bind(this._renderTitle, this));
            this.on('collapseChange', Y.bind(this._handleCollapse, this));


            this.processButtons();
        },
        processButtons: function() {
            var button, buttons, len, b, data = this.get('buttons');
            Y.each(data, function(i, k) {
                if (i.type == 'separator') {
                    this.addSeparator();
                } else if (i.group !== undefined) {
                    buttons = this.addButtonGroup(i);
                    if (buttons) {
                        len = buttons.length;
                        for(b = 0; b < len; b++) {
                            if (buttons[b]) {
        ...
        */
        addButtonGroup: function(oGroup) {
            ...
            var div = Y.Node.create('<div class="' + this.CLASS_PREFIX + '-group ' + this.CLASS_PREFIX + '-group-' + oGroup.group + '"></div>');
            if (oGroup.label) {
                var label = Y.Node.create('<h3>' + oGroup.label + '</h3>');
            ...
            Y.each(oGroup.buttons, function(i) {
                ...
                ul.appendChild(li);
                if ((i.type !== undefined) && i.type == 'separator') {
        ...
        */
        addButton: function(oButton, after) {
            ...
            if ((oButton.type == 'menu') || (oButton.type == 'split') || (oButton.type == 'select')) {
                if (Y.Lang.isArray(oButton.menu)) {
            ...
            */
            ...
            Y.each(oButton, function(v, k) {
                if (!this._toolbarConfigs[k]) {
                    _oButton[k] = v;
                }
            }, this);
            ...
            if (_oButton.menu) {
                if ((Y.Overlay) && (oButton.menu instanceof Y.Overlay)) {
                    oButton.menu.on('visibleChange', function() {
            ...
            if (skip) {
                ...
                tmp.get('node').set('tabIndex', '-1');
                tmp.get('node').set('role', 'button');
                if (!_oButton.id || !oButton.id) {
                ...
                */
                tmp.get('node').addClass(this.CLASS_PREFIX + '-' + tmp.get('value'));

                var icon = Y.Node.create('<span class="' + this.CLASS_PREFIX + '-icon"></span>');
                tmp.get('node').insertBefore(icon, tmp.get('node').get('firstChild'));
                /*
                ...
                */
                if (oButton.type == 'spin') {
                    if (!Y.Lang.isArray(oButton.range)) {
                        oButton.range = [ 10, 100 ];
                    }
                    this._makeSpinButton(tmp, oButton);
                }
                tmp.get('node').set('title', tmp.get('label'));
                if (oButton.type != 'spin') {
                    if ((Y.Overlay) && (_oButton.menu instanceof Y.Overlay)) {
                        var showPicker = function(ev) {
                            ...
                            if (exec) {
                                ...
                                tmp.getMenu().set('visible', !tmp.getMenu().get('visible'));
                            }
                            ev.halt();
                        };
                        ...
                        tmp.get('node').on('click', function(ev) {
                            ev.halt();
                        });
                    } else {
        ...
        */
        addSeparator: function(cont, after) {
            ...
            if (!this._sep) {
                Y.log('Separator does not yet exist, creating');
                this._sep = Y.Node.create('<span class="' + this.CLASS_SEPARATOR + '">|</span>');
            }
            Y.log('Separator does exist, cloning');
            var _sep = this._sep.cloneNode(true);
            this._sepCount++;

            _sep.addClass(this.CLASS_SEPARATOR + '-' + this._sepCount);
            if (after) {
                var nextSib = null;
                if (after.get) {
                    nextSib = after.get('element').nextSibling;
        ...
        */
        _createColorPicker: function(id) {
            var c = Y.get('#' + id + '_colors');
            if (c) {
               c.get('parentNode').removeChild(c);
            }
            var picker = Y.Node.create('<div class="yui-toolbar-colors" id="' + id + '_colors" style="display: none;"></div>');
            Y.get('body').appendChild(picker);

            this._colorPicker = picker;

            var html = '';
            Y.each(this._colorData, function(v, k) {
                html += '<a style="background-color: ' + k + '" href="#">' + k.replace('#', '') + '</a>';
            }, this);
            html += '<span><em>X</em><strong></strong></span>';
            picker.set('innerHTML', html);

            picker.on('mouseover', Y.bind(function(ev) {
                var picker = this._colorPicker;
                var em = picker.query('em');
                var strong = picker.query('strong');
                var tar = ev.target;
                if (tar.get('tagName').toLowerCase() == 'a') {
                    em.setStyle('backgroundColor', tar.getStyle('backgroundColor'));
                    strong.set('innerHTML', this._colorData['#' + tar.get('innerHTML')] + '<br>' + tar.get('innerHTML'));
                }
            }, this));
            picker.on('focus', function(ev) {
                ev.halt();
            });
            picker.on('click', function(ev) {
                ev.halt();
            });
            picker.on('mousedown', Y.bind(function(ev) {
                ev.halt();
                var tar = ev.target, data;
                if (tar.get('tagName').toLowerCase() == 'a') {
                    data = tar.get('innerHTML');
                    var retVal = this.fire('colorPickerClicked', { type: 'colorPickerClicked', button: this._colorPicker._button, color: data, colorName: this._colorData['#' + data] } );
                    var info = {
                        color: data,
        ...
        */
        _resetColorPicker: function() {
            var em = this._colorPicker.query('em');
            var strong = this._colorPicker.query('strong');
            em.setStyle('backgroundColor', 'transparent');
            strong.set('innerHTML', '');
        },
        /**
        ...
        */
        _makeColorButton: function(_oButton) {
            ...
            _oButton.type = 'color';
            _oButton.menu = new Y.Overlay({
                contentBox: '#' + this.get('id') + '_' + _oButton.value + '_menu',
                bodyContent: 'FOO',
                visible: false,
                zIndex: 999
            });
            _oButton.menu.render(this.get('cont'));
            _oButton.menu.set('visible', false);

            _oButton.menu.get('contentBox').addClass('yui-button-menu');
            _oButton.menu.get('contentBox').addClass('yui-color-button-menu');

            _oButton.menu.after('visibleChange', Y.bind(function() {
                _oButton.menu.set('align', {
                    node: this.getButtonById(_oButton.id).get('node'),
                    points: ['tl', 'bl']
                });
                ...
                if (_p.get('parentNode')) {
                    _p.get('parentNode').removeChild(_p);
                }
                _oButton.menu.get('contentBox').query('.yui-widget-bd').set('innerHTML', '');
                _oButton.menu.get('contentBox').query('.yui-widget-bd').appendChild(_p);
                this._colorPicker.setStyle('display', 'block');
            }, this));
        ...
        */
        _makeSpinButton: function(_button, oButton) {
            _button.get('node').addClass(this.CLASS_PREFIX + '-spinbutton');
            var self = this,
                _par = _button.get('node'), //parentNode of Button Element for appending child
                range = oButton.range,
                _b1 = Y.Node.create('<a href="#" tabIndex="-1" class="up" title="' + this.STR_SPIN_UP + '">' + this.STR_SPIN_UP + '</a>'),
                _b2 = Y.Node.create('<a href="#" tabIndex="-1" class="down" title="' + this.STR_SPIN_DOWN + '">' + this.STR_SPIN_DOWN + '</a>');
            ...
            var _intUp = function(ev) {
                ...
                if (!_button.get('disabled') && (ev.keyCode != 9)) {
                    ...
                    var label = Y.Lang.substitute(strLabel, { VALUE: _button.get('label') });
                    _button.set('title', label);
            ...
            //Handle arrow keys..
            _button.on('keydown', Y.bind(_intKeyUp, this));

            //Listen for the click on the up button and act on it
            //Listen for the click on the down button and act on it
            _b1.on('mousedown',function(ev) { ev.halt(); });
            _b2.on('mousedown',function(ev) { ev.halt(); });
            _b1.on('click', Y.bind(_intUp, this));
            _b2.on('click', Y.bind(_intDown, this));
        },
        /**
        * @protected
        * @method _buttonClick
        ...
        */
        _buttonClick: function(ev, info) {
            ...
            if (doEvent) {
                ...
                if (info.value) {
                    Y.log('fireEvent::' + info.value + 'Click');
                    retValue = this.fire(info.value + 'Click', { type: info.value + 'Click', target: this.get('element'), button: info });
                    if (retValue === false) {
                        fireNextEvent = false;
                    }
                }
                ...
                if (fireNextEvent) {
                    Y.log('fireEvent::buttonClick');
                    this.fire('buttonClick', { type: 'buttonClick', target: this.get('element'), button: info });
                }
            ...
            switch (ev.keyCode) {
                ...
                case 39:
                    ...
                    if (this._buttonList[this._navCounter]) {
                        var el = this._buttonList[this._navCounter].get('node');
                        if (this.browser.ie) {
                            el = this._buttonList[this._navCounter].get('node').query('a');
                        }
        ...
        */
        _handleFocus: function() {
            if (!this._keyNav) {
                ...
                this.get('node').on(ev, Y.bind(this._navigateButtons, this));
                this._keyNav = true;
                this._navCounter = -1;
            }
        },
        ...
        */
        selectButton: function(id, value) {
            var button = getButton.call(this, id);
            if (button && (button.get('type') == 'push')) {
                button.get('node').addClass('yui-button-selected');
                button.get('node').addClass('yui-button-' + button.get('value') + '-selected');
        ...
        */
        deselectButton: function(id) {
            var button = getButton.call(this, id);
            if (button) {
                button.get('node').removeClass('yui-button-selected');
                button.get('node').removeClass('yui-button-' + button.get('value') + '-selected');
                button.get('node').removeClass('yui-button-hover');
        ...
        */
        collapse: function(collapse) {
            var el = this._titlebar.query('span.collapse');
            if (collapse === false) {
                this.get('cont').get('parentNode').removeClass('yui-toolbar-container-collapsed');
                if (el) {
                    el.removeClass('collapsed');
                    el.set('title', this.STR_COLLAPSE);
                }
                this.fire('toolbarExpanded', { type: 'toolbarExpanded' });
            } else {
                if (el) {
                    el.addClass('collapsed');
                    el.set('title', this.STR_EXPAND);
                }
                this.get('cont').get('parentNode').addClass('yui-toolbar-container-collapsed');
                this.fire('toolbarCollapsed', { type: 'toolbarCollapsed' });
            }
        },
        /**
        * @method toString
    ...
    Y.namespace('Y.Toolbar');
    Y.Toolbar.Bar = Toolbar;
    ...
    var SimpleEditor = function(o) {
        Y.log('SimpleEditor Initalizing');

        var node = Y.get(o.node),
        id = node.get('id');
        if (!id) {
            id = Y.stamp(node);
            node.set('id', id);
        ...
        var element_cont = Y.Node.create('<div id="' + id + '_container"><div class="first-child"><div id="' + id + '_toolbar" class="toolbar_cont"></div><div class="wrapper"></div></div></div>');
        //Hack
        Y.get('body').appendChild(element_cont);
        o.element_cont = element_cont;
        o.editor_wrapper = element_cont.query('div.wrapper');
        o.toolbar_cont = element_cont.query('div.toolbar_cont');
        //Hack
        Y.get('body').removeChild(element_cont);

        if (!o.html) {
            o.html = '<html><head><title>{TITLE}</title><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /><base href="' + this._baseHREF + '"><style>{CSS}</style><style>{HIDDEN_CSS}</style><style>{EXTRA_CSS}</style></head><body onload="document.body._rteLoaded = true;">{CONTENT}</body></html>';
        }
        if (!o.css) {
            o.css = this._defaultCSS;
        }
        //console.log(o);


        SimpleEditor.superclass.constructor.apply(this, arguments);
    };
        ...
        */
        'height': {
            ...
            setter: function(height) {
                if (this._rendered) {
                    this.get('iframe').get('parentNode').setStyle('height', height);
                }
            }
        },
        /**
    ...
    Y.extend(SimpleEditor, Y.PortBase, {
        _handleSubmit: function() {
            var exec = this.get('handleSubmit');
            if (this.get('node').get('form')) {
                if (!this._formButtons) {
                    this._formButtons = [];
                }
                if (exec) {
                    this.get('node').get('form').on('submit', Y.bind(this._handleFormSubmit, this));

                    var i = this.get('node').get('form').query('input');
                    if (i) {
                        i.each(function(v, k) {
                            var type = v.get('type');
                            if (type && (type.toLowerCase() == 'submit')) {
                                v.on('click', Y.bind(this._handleFormButtonClick, this));
                                this._formButtons.push(v);
                            }
                        }, this);
                    }
                } else {
        ...
        _handleDOMPath: function() {
            var dompath = this.get('dompath');
            if (dompath && !this.dompath) {
                this.dompath = Y.Node.create('<div class="dompath" id="'+ this.get('node').get('id') + '_dompath"></div>');
                this.get('element_cont').get('firstChild').appendChild(this.dompath);
                if (this.get('iframe')) {
                    this._writeDomPath();
                }
            } else if (!dompath && this.dompath) {
                this.dompath.get('parentNode').removeChild(this.dompath);
                this.dompath = null;
            }
        },
        /**
        ...
        */
        _createIframe: function() {
            ...
            ifrmDom.id = this.get('node').get('id') + '_editor';
            var config = {
                border: '0',
                frameBorder: '0',
                marginWidth: '0',
            ...
            Y.each(config, function(v, k) {
                ifrmDom.setAttribute(k, v);
            });
            var isrc = 'javascript:;';
            if (this.browser.ie) {
                //isrc = 'about:blank';
        ...
        */
        _getDoc: function() {
            var value = false;
            try {
                var f = document.getElementById(this.get('iframe').get('id'));
                if (f && f.contentWindow.document) {
                    value = f.contentWindow.document;
                    return value;
                }
            } catch (e) {
                return false;
        ...
        */
        _initEditorEvents: function() {
            ...
            Y.Event.nativeAdd(doc, 'mouseup', Y.bind(this._handleMouseUp, this));
            Y.Event.nativeAdd(doc, 'mousedown', Y.bind(this._handleMouseDown, this));
            Y.Event.nativeAdd(doc, 'click', Y.bind(this._handleClick, this));
            Y.Event.nativeAdd(doc, 'dblclick', Y.bind(this._handleDoubleClick, this));
        ...
        */
        _removeEditorEvents: function() {
            ...
            Y.Event.nativeRremove(doc, 'mouseup', Y.bind(this._handleMouseUp, this));
            Y.Event.nativeRremove(doc, 'mousedown', Y.bind(this._handleMouseDown, this));
            Y.Event.nativeRremove(doc, 'click', Y.bind(this._handleClick, this));
            Y.Event.nativeRremove(doc, 'dblclick', Y.bind(this._handleDoubleClick, this));
            Y.Event.nativeRremove(doc, 'keypress', Y.bind(this._handleKeyPress, this));
            Y.Event.nativeRremove(doc, 'keyup', Y.bind(this._handleKeyUp, this));
            Y.Event.nativeRremove(doc, 'keydown', Y.bind(this._handleKeyDown, this));

            //Focus and blur..
            Y.Event.nativeRremove(win, 'focus', Y.bind(this._handleFocus, this));
            Y.Event.nativeRremove(win, 'blur', Y.bind(this._handleBlur, this));
        },
        ...
        */
        _initEditor: function(raw) {
            ...
            if (raw) {
                this.fire('editorContentReloaded', { type: 'editorreloaded'});
            } else {
                this.fire('editorContentLoaded', { type: 'editorLoaded' });
            }
            this._fixWebkitDivs();
            if (this.get('dompath')) {
                YAHOO.log('Delayed DomPath write', 'info', 'SimpleEditor');
                Y.later(150, this, this._writeDomPath);
            }
            var br = [];
            Y.each(this.browser, function(v, k) {
                if (v) {
                    br.push(k);
                }
            }, this);
            ...
            Y.DOM.addClass(this._getDoc().body, br.join(' '));
        ...
        */
        _setInitialContent: function(raw) {
            ...
            var value = ((this._textarea) ? this.get('node').get('value') : this.get('node').get('innerHTML')),
                doc = null;
            ...
            var html = Y.Lang.substitute(this.get('html'), {
            ...
            if (this.browser.ie || this.browser.webkit || this.browser.opera || (navigator.userAgent.indexOf('Firefox/1.5') != -1)) {
                ...
                this.get('iframe').set('src', 'data:text/html;charset=utf-8,' + encodeURIComponent(html));
            }
        ...
        * @description Calls _getSelectedElement and passes it through Y.get to get a Node reference.
        * @return {Y.Node} The currently selected element.
        */
        _getSelectedNode: function() {
            return Y.get(this._getSelectedElement());
        },
        /**
        * @private
        * @method _getDomPath
        ...
        */
        _writeDomPath: function() {
            ...
            //Prevent flickering
            if (this.dompath.get('innerHTML') != str) {
                this.dompath.set('innerHTML', str);
            }
        },
        /**
        * @private
        ...
        */
        _fixNodes: function() {
            try {
                ...
                Y.each(this.invalidHTML, function(v, k) {
                    if (k.toLowerCase() != 'span') {
                        var tags = doc.body.getElementsByTagName(k);
                        if (tags.length) {
        ...
        */
        _nodeChange: function(force) {
            ...
            var beforeNodeChange = this.fire('beforeNodeChange', { type: 'beforeNodeChange' });
            if (beforeNodeChange === false) {
                return false;
            }
            if (this.get('dompath')) {
                Y.later(0, this, this._writeDomPath);
            }
            ...
            if (!this.get('disabled')) {
                if (this.STOP_NODE_CHANGE) {
                    ...
                    if (fn_button) {
                        var family = fn_button._conf.data.initValue.label;
                    ...
                    if (fs_button) {
                        fs_button.set('label', fs_button._conf.data.initValue.label);
                    }

                    var hd_button = this.toolbar.getButtonByValue('heading');
                    if (hd_button) {
                        hd_button.set('label', hd_button._conf.data.initValue.label);
            ...
            this.fire('afterNodeChange', { type: 'afterNodeChange' });
        },
        /**
        * @private
        ...
        */
        _setupAfterElement: function() {
            if (!this.beforeElement) {
                this.beforeElement = Y.Node.create('<h2 class="yui-editor-skipheader" tabIndex="-1"></h2>');
                this.beforeElement.set('innerHTML', this.STR_BEFORE_EDITOR);
                this.get('element_cont').query('.first-child').insertBefore(this.beforeElement, this.toolbar.get('node').get('nextSibling'));
            }
            if (!this.afterElement) {
                this.afterElement = Y.Node.create('<h2 class="yui-editor-skipheader" tabIndex="-1">' + this.STR_LEAVE_EDITOR + '</h2>');
        ...
        */
        _disableEditor: function(disabled) {
            ...
            if (!this.get('disabled_iframe') && disabled) {
            ...
            if (disabled) {
                ...
                if (!this._mask) {
                    ...
                    if (this.browser.ie) {
                        this._mask.setStyle('height', height + 'px');
                    }
                ...
                if (this._mask) {
                    this._mask.get('parentNode').removeChild(this._mask);
                    this._mask = null;
                    if (this.toolbar) {
                        this.toolbar.set('disabled', false);
                    }
                    ...
                    this.focus();
                    Y.later(100, this, this.nodeChange);
                }
            }
        },
        /**
        ...
        */
        initializer: function() {
            Y.log('SimpleEditor initializer');
            ...
            this._handleInsert();
            this.set('saveEl', this.get('node'));
            this._handleSubmit();
            this.on('handleSubmitChange', Y.bind(this._handleSubmit, this));
            if (this.get('focusAtStart')) {
        ...
        */
        _getBlankImage: function() {
            ...
            if (!this._blankImageLoaded) {
                if (Y.Editor.Info.blankImage) {
                    this.set('blankimage', Y.Editor.Info.blankImage);
        ...
        */
        _handleFormSubmit: function(ev) {
            ...
            var form = this.get('node').get('form'),
                tar = this._formButtonClicked || false;

            form.detach('submit', Y.bind(this._handleFormSubmit, this));
            //console.log(form);
            /*
            if (this.browser.ie) {
                if (tar && !tar.disabled) {
        ...
        _handleAfterNodeChange: function() {
            ...
            for (var i = 0; i < path.length; i++) {
                ...
                if (tag.substring(0, 1) == 'h') {
                    if (hd_button) {
                        /*
                        for (var h = 0; h < hd_button._conf.data.initValue.menu.length; h++) {
                            if (hd_button._conf.data.initValue.menu.value[h].value.toLowerCase() == tag) {
            ...
            if (fn_button) {
                /*
                for (var b = 0; b < fn_button._conf.data.initValue.menu.value.length; b++) {
                ...
                */
                if (!validFont) {
                    family = fn_button._conf.data.initValue.label;
                }
            ...
            if (fs_button) {
                ...
                if ((fontsize === null) || isNaN(fontsize)) {
                    fontsize = fs_button._conf.data.initValue.label;
                }
        ...
        */
        render: function() {
            ...
            if (this.get('node')) {
                this._textarea = true;
                if (this.get('node').get('tagName').toLowerCase() !== 'textarea') {
        ...
        */
        _render: function() {
            YAHOO.log('_render');
            this.set('textarea', this.get('node'));
            ...
            if (!tbarConf) {
                tbarConf = this._defaultToolbar;
            }
            //Create Toolbar instance
            if (tbarConf instanceof Y.Toolbar.Bar) {
                this.toolbar = tbarConf;
                //Set the toolbar to disabled until content is loaded
                this.toolbar.set('disabled', true);
            } else {
                tbarConf.node = this.get('toolbar_cont');
                //Set the toolbar to disabled until content is loaded
                tbarConf.disabled = true;
                this.toolbar = new Y.Toolbar.Bar(tbarConf);
            }
            ...
            this.get('node').setStyle('left', '-9999px');
            this.get('element_cont').appendChild(this.get('node'));
            this.get('element_cont').setStyle('display', 'block');


            this.get('iframe').get('parentNode').addClass(this.CLASS_EDITABLE_CONT);
            ...
            this.get('iframe').get('parentNode').setStyle('height', this.get('height'));

            this.get('iframe').setStyle('width', '100%'); //WIDTH
            this.get('iframe').setStyle('height', '100%');

            Y.later(0, this, this._setupAfterElement);
            this.fire('afterRender', { type: 'afterRender' });
        },
        /**
        * @method execCommand
        ...
        */
        cmd_fontname: function(value) {
            ...
            if (selEl && selEl.tagName && !this._hasSelection() && !this._isElement(selEl, 'body') && !this.get('insert')) {
                Dom.setStyle(selEl, 'fontFamily', value);
                exec = false;
        ...
        cmd_fontsize: function(value) {
            ...
            if (this.browser.webkit) {
                if (this.currentElement[0]) {
                    if (el == this.currentElement[0]) {
                        go = false;
                        Dom.setStyle(el, 'fontSize', value);
                        this._selectNode(el);
                        this.currentElement[0] = el;
                    }
                }
            ...
            if (go) {
                if (!this._isElement(this._getSelectedElement(), 'body') && (!this._hasSelection())) {
                    ...
                    Dom.setStyle(this.currentElement, 'fontSize', value);
                } else {
    ...
    /* }}} */
        ...
        */
        _createCurrentElement: function(tagName, tagStyle) {
            ...
            if (!this._hasSelection()) {
                ...
                var _tmp = [], __tmp, __els = ['font', 'span', 'i', 'b', 'u', 'li'];
        ...
        */
        saveHTML: function() {
            ...
            if (this._textarea) {
                this.get('node').set('value', html);
            } else {
                this.get('node').set('innerHTML', html);
            }
            if (this.get('saveEl') !== this.get('node')) {
                var out = this.get('saveEl');
                if (out) {
                    if (out.get('tagName').toLowerCase() === 'textarea') {
                        out.set('value', html);
                    } else {
...
Y.Editor.Info = {
    ...
    */
    saveAll: function(form) {
        var i, e, items = Y.Editor.Info._instances;
        if (form) {
            Y.each(items, function(v, e) {
    ...
    /**
    * @method toString
    * @description Returns a string representing the Editor.Info.
    * @return {String}
    */
    toString: function() {
        var len = 0;
        Y.each(this._instances, function() {
            len++;
        });
...
}, 'gallery-2009.11.02-20' ,{requires:['node','gallery-port','substitute']});
