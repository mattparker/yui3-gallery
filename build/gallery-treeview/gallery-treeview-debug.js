  ...
  /**
    ...
    */
    ...
    collapseComplete: function(node) {
        Y.log("collapse complete: " + this.id);
        --this._animCount;
        this.fire("animComplete", {
                "node": node,
                "type": "collapse"
...
/**
    ...
    buildTreeFromMarkup: function (id) {
        Y.log('Building tree from existing markup');
        var build = function (markup) {
            ...
            for (el = Dom.getFirstChild(markup); el; el = Dom.getNextSibling(el)) {
                switch (el.tagName.toUpperCase()) {
                    ...
                    case 'UL':
                    case 'OL':
                        Y.log('ULs or OLs can only contain LI elements, not other UL or OL.  This will not work in some browsers','error');
                        config = {
                            type: 'text',
                            label: '',
        ...
        if (markup.length) {
            ...
            Y.log('Markup contains no UL or OL elements','warn');
        }
    },
  /**
  ...
  /**
    ...
    removeChildren: function(node) {
        ...
        Y.log("Removing children for " + node);
...
*/
  ...
  /**
    ...
    collapse: function() {
        ...
        if (false === ret) {
            Y.log("Collapse was stopped by the abstract onCollapse");
            return;
        }
        ...
        if (false === ret) {
            Y.log("Collapse was stopped by a custom event handler");
            return;
        }
    ...
    expand: function(lazySource) {
        ...
        if (!lazySource) {
            ...
            if (false === ret) {
                Y.log("Expand was stopped by the abstract onExpand");
                return;
            }
        ...
        if (false === ret) {
            Y.log("Expand was stopped by the custom event handler");
            return;
        }
        ...
        if (!this.childrenRendered) {
            Y.log("children not rendered yet");
            this.getChildrenEl().innerHTML = this.renderChildren();
        } else {
            Y.log("children already rendered");
        }
     ...
     */
    getStyle: function() {
        // Y.log("No children, " + " isDyanmic: " + this.isDynamic() + " expanded: " + this.expanded);
        if (this.isLoading) {
            Y.log("returning the loading icon");
            return "ygtvloading";
        } else {
    ...
    expandAll: function() {
        ...
        for (var i=0;i<l;++i) {
            ...
            if (c.isDynamic()) {
                Y.log("Not supported (lazy load + expand all)");
                break;
            } else if (! c.multiExpand) {
                Y.log("Not supported (no multi-expand + expand all)");
                break;
            } else {
                c.expand();
                c.expandAll();
    ...
    renderChildren: function() {
        ...
        Y.log("rendering children for " + this.index);
        ...
        if (this.isDynamic() && !this.dynamicLoadComplete) {
            ...
            if (this.dataLoader) {
                Y.log("Using dynamic loader defined for this node");
                ...
                Y.log("Using the tree-level dynamic loader");
    ...
    completeRender: function() {
        Y.log("completeRender: " + this.index + ", # of children: " + this.children.length);
        var sb = [];
    ...
    loadComplete: function() {
        Y.log(this.index + " loadComplete, children: " + this.children.length);
    ...
    getAncestor: function(depth) {
        if (depth >= this.depth || depth < 0)  {
            Y.log("illegal getAncestor depth: " + depth);
            return null;
        }
